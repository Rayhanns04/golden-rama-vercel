...
import { checkRate } from "../../src/services/hotel.service";
...
const Payment = (props) => {
  ...
  const { transaction, hotelDetail } = useSelector(
  ...
  const { data, isLoading } = useQuery(["checkRate", hotelDetail], () =>
    checkRate(hotelDetail?.rateKey)
  );
  ...
  const handlePromo = async (promoCode) => {
    const payment = await getPromoByCode(promoCode, "hotel");
    if (!payment)
      return toast({
    ...
    const payload = {
      promo: promoCode,
      totalPrice: transaction.totalTransaction,
      category: "hotel",
                ...
                <Stack spacing={5}>
                  {[
                    ...
                    },
                    {
                      t: "Total Pembayaran",
                      p: `IDR ${
                        isPromoAvailable?.available
                          ? convertRupiah(
                              transaction.totalTransaction -
                                parseInt(isPromoAvailable?.totalDiscount)
                            )
                          : convertRupiah(transaction.totalTransaction)
                      }`,
                    },
                    {
                      t: "Status Pembayaran",
          ...
          >
            <Box maxW={{ lg: "container.lg", xl: "container.xl" }} mx={"auto"}>
              ...
              >
                  ...
                  <Stack>
                    ...
                    (transaction?.promo_code &&
                      ...
                      <Text
                        ...
                        color="alert.failed"
                      >
                        Total transaksi anda masih belum mencukupi untuk
                ...
                >
                  {!isLoading && !isDesktop ? (
                    <Box p="24px" borderRadius="12px" bg="brand.blue.100">
                      <Card card={card} />
                    </Box>
                  ) : (
                    <Center>
